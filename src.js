 ...
 */
var userCountryOrigin = ""; // Get the country the user is browsing from
/**
 ...
 * @default false
 */
var processItemsOnCartPageCalled = false;
/**
 * Hidden div id for holding cartdata
 * @default '#yankeemall_inject_cartdata'
 */
var yankeeMallInjectCartdataDivId = "yankeemall_inject_cartdata";
/**
 * Div Id for __
 * @default #yankeemall_inject
 */
var yankeeMallInjectDivId = "yankeemall_inject";
/**
    ...
    injectScript(function () {
        chrome.storage.local.get("sites_info", function (res) {
            chrome.storage.local.get("saved_time", function (timeObj) {
                var dataSavedBefore = new Date().getTime() - timeObj["saved_time"];
                // Check if data is up to cacheTime
                if (dataSavedBefore < cacheTime && res["sites_data"]) {
                }
            });
        });
    });
...
function injectScript(callback) {
    ...
    var cartDataDiv = $("#" + yankeeMallInjectCartdataDivId);
    ...
    if (cartDataDiv.length > 0) {
        cartDataDiv.on("GET_CART_ITEMS", function () {
            var jsonData = $("#" + yankeeMallInjectCartdataDivId).text();
            if (jsonData) {
    ...
    var injectDiv = $("#" + yankeeMallInjectDivId);
    if (injectDiv.length === 0) {
        ...
        $("<script />", {
            ...
            src: src,
            id: yankeeMallInjectDivId + "_script",
            async: false
        }).appendTo($("head"));
    }
...
function getCartUrl() {
    ...
    if (typeof supportSites[host] !== "undefined") {
        if (typeof cartUrls === "string") {
            ...
            for (var y = 0; y < cartUrls.length; y++) {
                var temp = getLocation(cartUrls[y]);
                var temp1 = getLocation(cartUrls[y]);
...
function processItemsOnCartPage(url, html, domain) {
    ...
    if (url) {
        ...
        var cartUrl;
        if (windowHref.indexOf("?") == windowHref.length - 1 ||
        ...
        cartUrl = getCartUrl();
        if ((typeof jsonDataSites[windowHost] != "undefined" &&
            !html &&
            cartUrl) ||
 ...
 * supported sites using ajax to get cart items are called seperately to get cart items from ajax and the response is passed to processItemsOnCartPage
...
function getHTML() {
    ...
    var cartUrl = getCartUrl();
    var cartHost = getLocation(cartUrl);
    ...
    if (cartHost.hostname == windowHost &&
        ...
        axios
            ...
            var data = res.data;
            console.log(res);
            var contentType = res.headers["Content-Type"], as = string;
